...
// TestKustomizationReconcileHandler tests POST /api/fluxcd/kustomizations/{namespace}/{name}/reconcile
func TestKustomizationReconcileHandler(t *testing.T) {
	t.Run("should reject non-POST methods", func(t *testing.T) {
		...
		for _, method := range methods {
			t.Run(method, func(t *testing.T) {
				req := httptest.NewRequest(method, "/api/fluxcd/kustomizations/flux-system/flux-system/reconcile", nil)
				w := httptest.NewRecorder()

				// Act
				KustomizationReconcileHandler(w, req)
	...
	t.Run("should set Content-Type to application/json", func(t *testing.T) {
		// Arrange
		req := httptest.NewRequest(http.MethodPost, "/api/fluxcd/kustomizations/flux-system/flux-system/reconcile", nil)
		w := httptest.NewRecorder()
	...
	t.Run("should return 400 when name segment is missing", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPost, "/api/fluxcd/kustomizations/flux-system//reconcile", nil)
		w := httptest.NewRecorder()
	...
	t.Run("should accept name with hyphens in path", func(t *testing.T) {
		...
		req := httptest.NewRequest(
			http.MethodPost,
			"/api/fluxcd/kustomizations/my-namespace/my-kustomization-abc12/reconcile",
			nil,
		)
		w := httptest.NewRecorder()
	...
	t.Run("should return 404 when kustomization does not exist", func(t *testing.T) {
		...
		req := httptest.NewRequest(
			http.MethodPost,
			"/api/fluxcd/kustomizations/non-existent-ns/non-existent-kustomization-xyz/reconcile",
			nil,
		)
		w := httptest.NewRecorder()
...
// TestKustomizationReconcilePath tests path parsing with the reconcile suffix.
func TestKustomizationReconcilePath(t *testing.T) {
	...
	t.Run("should parse namespace and name correctly with reconcile suffix", func(t *testing.T) {
		// Arrange
		path := "/api/fluxcd/kustomizations/flux-system/flux-system/reconcile"
