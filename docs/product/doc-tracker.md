...
| 2026-07-02 | 공통 셸 CM7(네임스페이스 URL 딥링크, kube 스타일 `/namespaces/<이름>` 경로) AC·테스트 시나리오 추가, FluxCD 상세 경로도 kube 스타일로 이전(구 경로 리다이렉트, prd-fluxcd 경로 표기 갱신) — V7 커버, 단위·e2e 포함 | AC 52개 | AC 53개 전부 커버, **문서 체계 건강 유지** |
