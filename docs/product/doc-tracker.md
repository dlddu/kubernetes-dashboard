...
| 2026-07-15 | 위 재구조화의 e2e 회귀 수정 — FluxCD **클러스터 변경 스펙(FX3 reconcile·FX4 suspend/resume)** 을 FX2 목록 리더와 동일 파일(`fluxcd-kustomizations.spec.ts`)로 **재병합**. 파일 분리 시 `workers:4` 파일 단위 병렬로 공유 kind 클러스터 레이스 발생(namespace 필터 테스트 비결정 실패) | FX2·FX3·FX4 물리 분리가 격리를 깨뜨림 | 연결 AC 48 유지, 물리 파일은 FX3·FX4 가 FX2 파일에 공존(헤더로 식별) — cluster-stateful 스펙에 한해 파일 1:1 예외 |
