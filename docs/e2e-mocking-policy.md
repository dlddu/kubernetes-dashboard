...
mutation의 성공을 합성해 fixture 준비나 다른 spec의 오염을 피하는 것도 허용하지 않는다.
ERR는 필요한 실패만, LAT는 필요한 지연만 주입하고 이후 성공 경로는 실 backend로 전달한다.
변경할 자원은 전용 fixture/namespace로 격리하고 생성한 자원만 정리한다.
...
| 4 | `e2e/workloads-restart-confirm.spec.ts` | `**/api/deployments/dashboard-mock-policy/restart-policy-target/restart` (POST, 관측 후 continue) | `LAT` | "Restarting…" 전이 상태 관측까지 지연 필요. 해제 후 전용 0-replica Deployment의 restart annotation을 실 backend로 갱신하며 공유 pod는 재시작하지 않음. 실 응답은 즉시 완료돼 전이 관측 불가. |
...
| 28 | `e2e/argo-submit.spec.ts` | `**/api/argo/workflow-templates/submit-policy-template/submit` (POST, 첫 호출 500 → 재시도 continue) | `ERR` | submit 실패 시 에러 뷰·Retry 재시도 검증. 실클러스터가 요청 시점에 500을 내도록 만들 수 없어 첫 실패만 주입. 재시도는 전용 fixture에 실 submit하고 생성된 workflow를 조회·검증한 뒤 해당 이름만 삭제. |
| 29 | `e2e/argo-submit.spec.ts` | `**/api/argo/workflow-templates/submit-policy-template/submit` (POST, 관측 후 continue) | `LAT` | in-flight(확인 버튼 비활성·스피너) 관측까지 지연 필요. 실 응답은 즉시 완료돼 관측 불가. 해제 후 전용 fixture에 실 submit하고 생성된 workflow를 조회·검증한 뒤 해당 이름만 삭제. |

### mutation fixture 격리와 검증

위 #4·#28·#29는 `test/fixtures/mock-policy-mutation-fixtures.yaml`을 사용한다.
`apply-all.sh`가 `dashboard-mock-policy` namespace, `submit-policy-template`,
`restart-policy-target`을 설치한다. WorkflowTemplate은 suspend 상태로 정의되고,
Deployment는 replicas 0으로 정의돼 다른 spec이 관측하는 pod를 변경하지 않는다.
테스트는 실제 자원 생성/조회/삭제 또는 실제 restart 요청을 검증한다. Workflow 실행 완료나
pod rollout은 이 세 UI 시나리오의 검증 범위가 아니다.

Argo 테스트는 실 submit 응답의 namespace·이름을 확인하고, API로 실재 자원과 template을
조회·검증한 뒤 `finally`에서 **그 응답이 반환한 이름만** 삭제하고 404까지 확인한다.
LAT의 지연은 UI 어서션의 성공/실패와 관계없이 해제되며, 실 응답을 기다린 뒤 정리한다.
namespace 전체 삭제나 공유 `simple-template` workflow 삭제는 하지 않는다.
...
> 모킹이라 `ERR`/`LAT`/`ABS` 어디에도 들지 않고 명시적 불허에 해당해, 등재가 아니라 제거가
